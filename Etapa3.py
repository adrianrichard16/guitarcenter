...
# -------------------------------------------------------------------
class Producto:
    def __init__(self, codigo, descripcion, origen, cantidad, precio):
        self.codigo = codigo
    ...
    def modificar(self, nueva_descripcion, nueva_origen, nueva_cantidad, nuevo_precio):
...
class Inventario:
    ...
    def agregar_producto(self, codigo, descripcion, origen, cantidad, precio):
        producto_existente = self.consultar_producto(codigo)
        if producto_existente:
            return jsonify({'message': 'Ya existe un producto con ese código.'}), 400

        #nuevo_producto = Producto(codigo, descripcion, cantidad, precio)
        self.cursor.execute("INSERT INTO productos VALUES (?, ?, ?, ?)", (codigo, descripcion, origen, cantidad, precio))
    ...
    def consultar_producto(self, codigo):
        ...
        if row:
            codigo, descripcion, origen, cantidad, precio = row
            return Producto(codigo, descripcion, origen, cantidad, precio)
        return None

    def modificar_producto(self, codigo, nueva_descripcion, nueva_origen, nueva_cantidad, nuevo_precio):
        producto = self.consultar_producto(codigo)
        if producto:
            producto.modificar(nueva_descripcion, nueva_origen, nueva_cantidad, nuevo_precio)
            self.cursor.execute("UPDATE productos SET descripcion = ?, origen = ?, cantidad = ?, precio = ? WHERE codigo = ?",
                                (nueva_descripcion, nueva_origen, nueva_cantidad, nuevo_precio, codigo))
            self.conexion.commit()
    ...
    def listar_productos(self):
        ...
        for row in rows:
            codigo, descripcion, origen, cantidad, precio = row
            producto = {'codigo': codigo, 'descripcion': descripcion, 'origen': origen, 'cantidad': cantidad, 'precio': precio}
...
class Carrito:
    ...
    def agregar(self, codigo, cantidad, inventario):
        ...
        nuevo_item = Producto(codigo, producto.descripcion, producto.origen, cantidad, producto.precio)
    ...
    def mostrar(self):
        productos_carrito = []
        for item in self.items:
            producto = {'codigo': item.codigo, 'descripcion': item.descripcion, 'origen': item.origen, 'cantidad': item.cantidad,
                        'precio': item.precio}
...
def agregar_producto():
    ...
    return inventario.agregar_producto(codigo, descripcion, origen, cantidad, precio)
...
def modificar_producto(codigo):
    ...
    nueva_origen = request.json.get('origen')
    nueva_cantidad = request.json.get('cantidad')
    nuevo_precio = request.json.get('precio')
    return inventario.modificar_producto(codigo, nueva_descripcion, nueva_origen, nueva_cantidad, nuevo_precio)
